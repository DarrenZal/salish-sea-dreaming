#!/usr/bin/env python3
"""
Anchor graph spine v004 round nature mask.
...
NATURE_RADIAL_MASK_BLUR_PX = 1.15
...
def render_frame(
    ...
    *,
    ...
    if debug:
        base.draw_text(frame, f"anchor_graph_spine_v004_round_nature_mask t={time_seconds:05.2f}", (42, 56), LABEL, scale=0.50)
    return frame, info
...
def save_key_stills(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, str]:
    ...
    for key, time_seconds in KEY_STILL_TIMES:
        ...
        path = STILLS_DIR / f"anchor_graph_spine_v004_round_nature_mask_{key}.png"
        cv2.imwrite(str(path), frame)
        outputs[f"{key}_still"] = str(path.relative_to(OUT_DIR))
    peak_frame, _info = render_frame(12.8, nature, raven)
    peak_path = PEAK_DIR / "anchor_graph_spine_v004_round_nature_mask_nature_portal_peak.png"
...
def make_contact_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    path = OUT_DIR / "anchor_graph_spine_v004_round_nature_mask_contact_sheet.png"
...
def make_v003_v004_comparison_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    panels: list[np.ndarray] = []
    for label_prefix, time_seconds in (("Nature hold", 20.0), ("Nature return", 70.5)):
        v003 = read_video_frame_at(V003_MP4, time_seconds)
        v004, _info = render_frame(time_seconds, nature, raven)
        panels.append(labeled_panel(v003, f"v003 {label_prefix} t={time_seconds:04.1f}s: softened square source"))
        panels.append(labeled_panel(v004, f"v004 {label_prefix} t={time_seconds:04.1f}s: shared radial source mask"))
    sheet = cv2.vconcat([cv2.hconcat(panels[:2]), cv2.hconcat(panels[2:])])
    path = OUT_DIR / "anchor_graph_spine_v003_v004_nature_comparison_sheet.png"
    cv2.imwrite(str(path), sheet)
    return path
...
def make_debug_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    panel_specs = (
        ("portal aperture still uses center/ray mask", 12.5),
        ("radial full Nature hold", 20.0),
        ("collapse starts from radial Nature mask", 28.0),
        ("return uses same radial Nature mask", 70.5),
    )
    panels: list[np.ndarray] = []
    for label, time_seconds in panel_specs:
        ...
        if np.count_nonzero(nature_mask) > 0:
            wave_v002.blend_mask(frame, mask_edge(nature_mask), MUTED_TEAL, 0.52)
            cv2.circle(
                frame,
                (round(ANCHOR_CENTER_PX[0]), round(ANCHOR_CENTER_PX[1])),
                round(NATURE_RADIAL_OUTER_RADIUS_PX),
                rgb_to_bgr(SOFT_GOLD),
                2,
                lineType=cv2.LINE_AA,
            )
        cv2.circle(
    ...
    candidate_panels: list[np.ndarray] = []
    for label, full_mult, outer_mult in NATURE_RADIAL_MASK_CANDIDATES:
        frame, _sources = render_wave_base(20.0)
        mask = nature_radial_hold_mask_for(nature, full_mult, outer_mult)
        mask = nature_presentation_mask(nature, mask)
        wave_v002.alpha_over(frame, nature.bgr, mask, 0.98)
        wave_v002.blend_mask(frame, mask_edge(mask), MUTED_TEAL, 0.58)
        full_radius = NATURE_OUTER_RAY_EXTENT_PX * full_mult
        outer_radius = NATURE_OUTER_RAY_EXTENT_PX * outer_mult
        cv2.circle(frame, (round(ANCHOR_CENTER_PX[0]), round(ANCHOR_CENTER_PX[1])), round(full_radius), rgb_to_bgr(PORTAL_GLOW), 1, lineType=cv2.LINE_AA)
        cv2.circle(frame, (round(ANCHOR_CENTER_PX[0]), round(ANCHOR_CENTER_PX[1])), round(outer_radius), rgb_to_bgr(SOFT_GOLD), 2, lineType=cv2.LINE_AA)
        panel = cv2.resize(frame, (640, 360), interpolation=cv2.INTER_AREA)
        base.draw_text(panel, f"{label}: full {full_mult:.2f}x, outer {outer_mult:.2f}x", (18, 34), LABEL, scale=0.32)
        base.draw_text(panel, "rings show full-strength radius and outer fade radius", (18, 62), LABEL, scale=0.28)
        candidate_panels.append(panel)
    candidate_row = cv2.hconcat(candidate_panels)

    mask_panel = np.full((360, 1920, 3), rgb_to_bgr(DEEP_GROUND), dtype=np.uint8)
    columns = (
        ("source alpha", nature.alpha),
        ("selected radial falloff", radial_falloff_mask(NATURE_RADIAL_FULL_RADIUS_PX, NATURE_RADIAL_OUTER_RADIUS_PX, blur_px=NATURE_RADIAL_MASK_BLUR_PX)),
        ("final Nature hold mask", nature_full_hold_mask(nature)),
    )
    for idx, (label, mask) in enumerate(columns):
        x0 = idx * 640
        colored = cv2.applyColorMap(cv2.resize(mask, (640, 360), interpolation=cv2.INTER_AREA), cv2.COLORMAP_TURBO)
        mask_panel[:, x0 : x0 + 640] = cv2.addWeighted(mask_panel[:, x0 : x0 + 640], 0.24, colored, 0.76, 0)
        cv2.circle(mask_panel, (x0 + 320, 180), round(ANCHOR_RADIUS_PX / 3.0), rgb_to_bgr(PORTAL_GLOW), 2, lineType=cv2.LINE_AA)
        cv2.circle(mask_panel, (x0 + 320, 180), round(NATURE_RADIAL_OUTER_RADIUS_PX / 3.0), rgb_to_bgr(SOFT_GOLD), 2, lineType=cv2.LINE_AA)
        base.draw_text(mask_panel, label, (x0 + 24, 42), LABEL, scale=0.38)
    base.draw_text(
        mask_panel,
        f"selected Nature radial mask: outer ray {NATURE_OUTER_RAY_EXTENT_PX:.1f}px, full {NATURE_RADIAL_FULL_RADIUS_PX:.1f}px, outer fade {NATURE_RADIAL_OUTER_RADIUS_PX:.1f}px, falloff {NATURE_RADIAL_FALLOFF_PX:.1f}px",
        (42, 326),
        LABEL,
        scale=0.35,
    )
    ...
    sheet = cv2.vconcat([actual_row, candidate_row, mask_panel, timeline])
    path = DEBUG_DIR / "anchor_graph_spine_v004_round_nature_mask_radial_mask_debug.png"
    cv2.imwrite(str(path), sheet)
    return path


def render_clip(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, object]:
    output_path = OUT_DIR / "anchor_graph_spine_v004_round_nature_mask.mp4"
    ...
    try:
        for frame_idx in range(N_FRAMES):
            ...
            if (frame_idx + 1) % FPS == 0:
                print(f"  anchor_graph_spine_v004_round_nature_mask.mp4 {frame_idx + 1}/{N_FRAMES}", flush=True)
    finally:
        writer.close()
    if peak_info is None:
        _frame, peak_info = render_frame(12.8, nature, raven, collect_info=True)
    return {
        "filename": "anchor_graph_spine_v004_round_nature_mask.mp4",
...
def write_manifest(
    ...
    manifest = {
        "project": "anchor_graph_spine_v004_round_nature_mask",
        ...
        "renderer": "scripts/anchor_graph_spine_v004_round_nature_mask.py",
        "renderer_sha256": sha256(ROOT / "scripts" / "anchor_graph_spine_v004_round_nature_mask.py"),
        ...
        "tested_edges": [
            {
                "edge_id": "wave_to_nature_cosmic_sun_spine_v004_round_nature_mask",
                "edge_type": "wave_to_artwork",
                ...
                "artifact_path": str((OUT_DIR / "anchor_graph_spine_v004_round_nature_mask.mp4").relative_to(ROOT)),
                "caveats": [
            ...
            {
                "edge_id": "nature_cosmic_sun_to_raven_sun_spine_v004_round_nature_mask",
                "edge_type": "artwork_to_artwork",
            ...
            {
                "edge_id": "raven_sun_to_cymatic_to_nature_return_spine_v004",
                "edge_type": "artwork_to_wave_atmosphere",
                "status": "internal_composition_pass",
        ...
        "deliverables": {
            ...
            "v003_v004_comparison_sheet": str(comparison_path.relative_to(OUT_DIR)),
            "radial_mask_debug_sheet": str(debug_path.relative_to(OUT_DIR)),
            "source_provenance_note": str(provenance_note.relative_to(OUT_DIR)),
            "readme": "README.md",
            **stills,
        },
    }
    path = OUT_DIR / "anchor_graph_spine_v004_round_nature_mask_manifest.json"
...
def write_readme(nature: ArtworkLayer, raven: ArtworkLayer, clip_summary: dict[str, object]) -> Path:
    lines = [
        ...
        "## Purpose",
        "",
        "This packet is the planned endpoint polish pass for the solo spine iteration. It preserves the v003 72 second loop architecture and fixes the remaining square-boundary/card read in the full Nature hold by replacing the raster-bounds presentation edge with a shared radial mask centered on the Nature sun-disc anchor.",
        "",
        ...
        "- 14-24s: full `Nature_Cosmic_Sun.svg` authored artwork resolves and holds through the radial whole-source presentation mask.",
        ...
        "## Deliverables",
        "",
        "- `anchor_graph_spine_v004_round_nature_mask.mp4`: 1920x1080, 24 fps, 72 seconds.",
        "- `anchor_graph_spine_v004_round_nature_mask_contact_sheet.png`: labeled timestamp sequence sheet.",
        ...
        "- `anchor_graph_spine_v003_v004_nature_comparison_sheet.png`: v003/v004 comparison focused on Nature hold and Nature return.",
        "- `debug_stills/anchor_graph_spine_v004_round_nature_mask_radial_mask_debug.png`: radial mask debug sheet with smaller/selected/larger still-only tests.",
        "- `anchor_graph_spine_v004_round_nature_mask_manifest.json`: sources, hashes, anchors, timings, opacity/blend settings, Raven stabilization, and boundary flags.",
        "- `SOURCE_PROVENANCE.md`: local source provenance and boundary note.",
        "",
        "## Honest Verdict",
        "",
        "v004 directly improves the v003 Nature card read. At the initial full hold and the final return, the same authored Nature raster is now presented through a round field centered on the sun-disc anchor. The sun face, outward rays, and enough cream/yellow background remain visible, while the square corners no longer dominate the composition.",
        "",
        "The mask is not perfect invisibility; a close look still shows this is a full rectangular source being presented through a radial window. But the dominant read is no longer a flat square card, and the falloff now feels connected to the portal/cymatic architecture. The selected radius keeps the ray tips intact; the larger debug variant preserves more cream field but starts to reintroduce a boundary-heavy read.",
        "",
        "Everything else is intentionally v003: CairoSVG source-faithful Nature rasterization, the Raven stable hold/frozen transform, black-disc presentation-alpha guard, Raven collapse, cymatic regather, and full 72 second loop timing. If this passes Austin review, this should stop the solo spine iteration and become the core artifact for the Austin conversation.",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render anchor graph spine v004 round nature mask.")
    ...
    comparison_path = make_v003_v004_comparison_sheet(nature, raven)
    ...
    if args.preview:
        ...
        clip_summary = {
            "filename": "anchor_graph_spine_v004_round_nature_mask.mp4",
            "mp4": "anchor_graph_spine_v004_round_nature_mask.mp4",
            "duration_seconds": DURATION_SECONDS,
            "fps": FPS,
            "frame_count": N_FRAMES,
            "peak_time_seconds": 12.8,
        ...
        print("Rendering anchor_graph_spine_v004_round_nature_mask.mp4", flush=True)
