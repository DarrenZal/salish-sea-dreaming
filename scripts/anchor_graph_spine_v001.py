...
Raven Sun reveal -> Raven hold -> return to wave/water atmosphere.
...
def render_frame(
    ...
    *,
    ...
    raven_mask = raven_mask_at(time_seconds, raven)
    r_opacity = raven_opacity(time_seconds)
    if r_opacity > 0.0:
        wave_v002.alpha_over(frame, raven.bgr, raven_mask, r_opacity)
    ...
    if debug:
        base.draw_text(frame, f"anchor_graph_spine_v001 t={time_seconds:05.2f}", (42, 56), LABEL, scale=0.50)
    return frame, info
...
def save_key_stills(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, str]:
    ...
    for key, time_seconds in KEY_STILL_TIMES:
        ...
        path = STILLS_DIR / f"anchor_graph_spine_v001_{key}.png"
        cv2.imwrite(str(path), frame)
        outputs[f"{key}_still"] = str(path.relative_to(OUT_DIR))
    peak_frame, _info = render_frame(12.8, nature, raven)
    peak_path = PEAK_DIR / "anchor_graph_spine_v001_nature_portal_peak.png"
...
def make_contact_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    path = OUT_DIR / "anchor_graph_spine_v001_contact_sheet.png"
...
def make_timeline_panel(width: int, height: int) -> np.ndarray:
    ...
    segments = (
        ...
        ("Raven hold", 44.0, 54.0, (180, 168, 130)),
...
def make_debug_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    r_mask = raven_mask_at(38.0, raven)
    ...
    path = DEBUG_DIR / "anchor_graph_spine_v001_anchor_mask_timeline_debug.png"
    cv2.imwrite(str(path), sheet)
    return path


def render_clip(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, object]:
    output_path = OUT_DIR / "anchor_graph_spine_v001.mp4"
    ...
    try:
        for frame_idx in range(N_FRAMES):
            ...
            if (frame_idx + 1) % FPS == 0:
                print(f"  anchor_graph_spine_v001.mp4 {frame_idx + 1}/{N_FRAMES}", flush=True)
    finally:
        writer.close()
    if peak_info is None:
        _frame, peak_info = render_frame(12.8, nature, raven, collect_info=True)
    return {
        "filename": "anchor_graph_spine_v001.mp4",
...
def write_manifest(
    ...
    manifest = {
        "project": "anchor_graph_spine_v001",
        ...
        "renderer": "scripts/anchor_graph_spine_v001.py",
        "renderer_sha256": sha256(ROOT / "scripts" / "anchor_graph_spine_v001.py"),
        ...
        "timings": [
            ...
            {
                "segment": "Raven Sun reveal from shared anchor",
                ...
                "transition_method": "source-aligned masks and opacity on whole source artworks",
            },
            {
                "segment": "Raven Sun hold",
                "time_seconds": [44.0, 54.0],
                "edge_type": "artwork_hold",
                "anchors_used": ["animal_bird_raven_sun.central_sun_disc"],
                "spatial_note": "Kept as a 2D whole-source hold in this composition; existing 2.5D probe is referenced, not imported, to preserve spine continuity.",
            },
            {
        ...
        "tested_edges": [
            {
                "edge_id": "wave_to_nature_cosmic_sun_spine_v001",
                "edge_type": "wave_to_artwork",
                ...
                "artifact_path": str((OUT_DIR / "anchor_graph_spine_v001.mp4").relative_to(ROOT)),
                "caveats": [
            ...
            {
                "edge_id": "nature_cosmic_sun_to_raven_sun_spine_v001",
                "edge_type": "artwork_to_artwork",
                ...
                "artifact_path": str((OUT_DIR / "anchor_graph_spine_v001.mp4").relative_to(ROOT)),
                "caveats": [
                    "Transition uses whole-source alignment and masks only.",
                    "Austin review is required before any public or show use.",
                ],
            },
            {
                "edge_id": "raven_sun_to_wave_return_spine_v001",
                "edge_type": "artwork_to_wave_atmosphere",
                "status": "internal_composition_pass",
    ...
    path = OUT_DIR / "anchor_graph_spine_v001_manifest.json"
...
def write_readme(nature: ArtworkLayer, raven: ArtworkLayer, clip_summary: dict[str, object]) -> Path:
    lines = [
        ...
        "## Purpose",
        "",
        "This packet is the first longer internal anchor-graph composition pass. It connects the validated pieces into one continuous installation spine: wave atmosphere -> Nature Cosmic Sun portal -> shared sun-disc anchor -> Raven Sun reveal -> Raven atmospheric hold -> return to wave/water atmosphere.",
        "",
        "Austin's source SVGs remain whole authored artworks. The shared sun-disc and eight-ray records are alignment metadata only. They are not renderable primitives, extracted motifs, atom records, or new SVG assets.",
        "",
        "## Sources",
        "",
        ...
        "- 32-44s: Raven Sun emerges from the same center/radius through source-aligned masks.",
        "- 44-54s: Raven Sun holds as the whole authored destination artwork.",
        "- 54-60s: the source recedes back toward wave/water atmosphere, with a ripple cue around the same center.",
        "",
        "## Deliverables",
        "",
        "- `anchor_graph_spine_v001.mp4`: 1920x1080, 24 fps, 60 seconds.",
        "- `anchor_graph_spine_v001_contact_sheet.png`: 12-frame sequence sheet.",
        "- `stills/`: wave gather, Nature portal, Nature hold, shared anchor, Raven transition, Raven hold, and return stills.",
        "- `debug_stills/anchor_graph_spine_v001_anchor_mask_timeline_debug.png`: anchor, mask, and timeline diagnostics.",
        "- `anchor_graph_spine_v001_manifest.json`: sources, hashes, anchors, timings, edge types, and deliverables.",
        "- `SOURCE_PROVENANCE.md`: local source provenance and boundary note.",
        "",
        "## Honest Verdict",
        "",
        "This reads more like a single installation passage than the earlier isolated probes. The longer Nature portal hold gives the wave-to-artwork alignment time to register, and the Nature-to-Raven handoff is carried by the shared sun-disc center/radius instead of a plain crossfade.",
        "",
        "The roughest part is the Raven hold. I kept it 2D in this spine rather than importing the existing 2.5D layer treatment, because the layer treatment would add another rendering contract inside the first long composition pass. Raven still has atmospheric support, but a later v002 could integrate the parallax probe more carefully.",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render anchor graph spine v001.")
    ...
    if args.preview:
        ...
        clip_summary = {
            "filename": "anchor_graph_spine_v001.mp4",
            "mp4": "anchor_graph_spine_v001.mp4",
            "duration_seconds": DURATION_SECONDS,
            "fps": FPS,
            "frame_count": N_FRAMES,
            "peak_time_seconds": 12.8,
        ...
        print("Rendering anchor_graph_spine_v001.mp4", flush=True)
        clip_summary = render_clip(nature, raven)

    manifest = write_manifest(input_status, nature, raven, clip_summary, stills, contact_path, debug_path, provenance_note)
