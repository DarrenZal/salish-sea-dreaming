#!/usr/bin/env python3
"""
abstract_radiating_primitive_ripples_v001

Generated Austin-style / Coast Salish-style primitive phrase ripple layer for
...
Default outputs:
  track2-deterministic/morph_outputs_INTERNAL/
    abstract_radiating_primitive_ripples_v001_2026-05-23/
      abstract_radiating_primitive_ripples_v001_overlay_black.mp4
      abstract_radiating_primitive_ripples_v001_alpha_prores4444.mov
      abstract_radiating_primitive_ripples_v001_over_moonfish-water.mp4
      abstract_radiating_primitive_ripples_v001_contact_sheet.png
      README.md
      manifest.json
"""
...
PROJECT = "abstract_radiating_primitive_ripples_v001"
OUT_DIR = (
    ROOT
    / "track2-deterministic"
    / "morph_outputs_INTERNAL"
    / f"{PROJECT}_2026-05-23"
)
...
def render_ripple_system(canvas: np.ndarray, system: RippleSystem, frame_index: int, n_frames: int) -> None:
    t = frame_index / FPS
    q = ((t / system.cycle_seconds) + system.phase) % 1.0
    ...
    breathe = 1.0 + 0.055 * math.sin(TAU * (q * 2.0 + system.phase * 1.7))
    # No construction/debug rings: the water feel is carried by the staggered
    # transparency, glow, and slow radial drift of the authored primitives.

    # 1. Inner circular pulse.
    circle_t = float(np.clip(q / 0.52, 0.0, 1.0))
    circle_fade = smoothstep(0.00, 0.08, q) * smoothstep(0.60, 0.36, q)
    if circle_fade > 0.002:
        circle_r = (20.0 + circle_t * 285.0) * px * (0.98 + 0.04 * math.sin(TAU * q * 3.0))
        draw_ring(
            canvas,
            ox,
            oy,
            circle_r,
            (8.0 + 3.5 * (1.0 - circle_t)) * px,
            _palette(system, "circle"),
            system.intensity * 0.88 * circle_fade,
        )
        if q < 0.18:
            draw_disc(canvas, ox, oy, (18.0 + 18.0 * circle_t) * px, _palette(system, "circle"), system.intensity * 0.42 * circle_fade)

    # 2. Middle ring of outward-cupping crescents.
    cres_t = (q - 0.12) / 0.70
    if 0.0 <= cres_t <= 1.0:
        cwin = smoothstep(0.0, 0.11, cres_t) * smoothstep(1.0, 0.74, cres_t)
        cdist_fade = 1.0 - 0.46 * ease_out_sine(cres_t)
        c_intensity = system.intensity * 0.82 * cwin * cdist_fade
        ring_r = 150.0 * px + ease_out_sine(cres_t) * (max_radius * 0.63)
        for k in range(system.count):
            theta0 = TAU * (k / system.count) + system.angle_offset
            wobble_phase = q * 1.15 + k * 0.137 + system.phase
            theta = theta0 + 0.035 * math.sin(TAU * wobble_phase)
            radial_wobble = 15.0 * px * math.sin(TAU * (q * 1.6 + k * 0.091))
            cx = ox + math.cos(theta) * (ring_r + radial_wobble)
            cy = oy + math.sin(theta) * (ring_r + radial_wobble)
            size = crescent_size_base * breathe * (0.92 + 0.10 * math.sin(TAU * (q * 2.1 + k / system.count)))
            draw_crescent(canvas, cx, cy, size, theta, _palette(system, "crescent"), c_intensity)

    # 3. Outer ring of outward-pointing curved trigons.
    tri_t = (q - 0.28) / 0.72
    if 0.0 <= tri_t <= 1.0:
        twin = smoothstep(0.0, 0.12, tri_t) * smoothstep(1.0, 0.78, tri_t)
        tdist_fade = 1.0 - 0.56 * ease_out_sine(tri_t)
        t_intensity = system.intensity * 0.72 * twin * tdist_fade
        ring_r = 245.0 * px + ease_out_sine(tri_t) * (max_radius * 0.77)
        for k in range(system.count):
            theta0 = TAU * ((k + 0.46) / system.count) + system.angle_offset
            theta = theta0 + 0.025 * math.sin(TAU * (q * 1.0 + k * 0.073 + system.phase))
            radial_wobble = 18.0 * px * math.cos(TAU * (q * 1.33 + k * 0.117))
            cx = ox + math.cos(theta) * (ring_r + radial_wobble)
            cy = oy + math.sin(theta) * (ring_r + radial_wobble)
            size = trigon_size_base * breathe * (0.96 + 0.08 * math.sin(TAU * (q * 1.8 + k / max(1, system.count - 1))))
            draw_trigon(canvas, cx, cy, size, theta, _palette(system, "trigon"), t_intensity)
...
def make_composite(layer_mp4: Path, outdir: Path, n_frames: int) -> Path | None:
    if not MOONFISH_FOOTAGE.is_file():
        print(f"  composite skipped: missing footage {MOONFISH_FOOTAGE}")
        return None
    ...
    fc = (
        f"[0:v]{crop},fps={FPS},trim=duration={duration:.6f},setpts=PTS-STARTPTS,"
        ...
        f"colorchannelmixer=rr=0.68:gg=0.68:bb=0.68[lay];"
...
def write_readme(outdir: Path, manifest: dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "## Visual Intent",
        "",
        "A loopable UHD visual layer where authored primitive syntax behaves like soft pond ripples on dark water. Each ripple event starts from an origin point, then radiates as a circle pulse, an outward-cupping crescent ring, and an outward-pointing trigon ring.",
        "",
        ...
        f"- Moonfish/water composite base: `{MOONFISH_FOOTAGE.relative_to(ROOT)}`",
        "",
        "## Render Specs",
        "",
        ...
        "- Motion: authored radial ripple phrases, staggered across four origins",
    ...
    lines += [
        ...
        "## Shape Notes",
        "",
        "- Crescents are tapered arc blades with two arc-bounded sides and sharp cusps; their cups open outward along the ripple radius.",
        "- Trigons are generated as concave-sided, arc-bounded directional heads; they point outward and avoid straight polygon icon geometry.",
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "visual_intent": "Austin-style / Coast Salish-style authored primitive syntax radiating like soft pond ripples over dark moonfish/water mood.",
        "source_references": {
