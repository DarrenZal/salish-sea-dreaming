#!/usr/bin/env python3
"""
Abstract cymatic primitive emergence v001.
...
from overlapping construction circles placed at procedural wave source and
antinode positions. The wave field supplies water atmosphere, drift, and
breathing; the peak geometry is not an amplitude-threshold blob layer.
...
OUT_DIR = (
    ...
    / "abstract_cymatic_primitive_emergence_v001_2026-05-22"
)
STILLS_DIR = OUT_DIR / "stills"
DEBUG_DIR = OUT_DIR / "debug"
LOOP_DIR = OUT_DIR / "loop_diagnostics"

PROJECT = "abstract_cymatic_primitive_emergence_v001"
FPS = 24
...
KEY_STILLS = (
    ...
    ("construction_circles_implied", 14.0),
    ("two_circle_lens_emergence", 26.0),
    ("three_circle_trigon_cells", 40.0),
...
class PrimitiveEmergenceRenderer:
    def __init__(self, config: RenderConfig) -> None:
        ...
        self.construction_radius = 250.0 * self.scale
        self.outer_source_radius = 435.0 * self.scale
        ...
        self.field_boundary = self._radial_falloff(600.0 * self.scale, 110.0 * self.scale)
    ...
    def _make_sources(self) -> list[WaveSource]:
        s = self.scale
        sources = [
            WaveSource("origin_00", "center_antinode", 0.0, 0.0, 1.12, 250.0 * s, 9, 0.0, 920.0 * s, 0, 0, 0.0, 0.0)
        ]
        for idx in range(6):
            angle = -math.pi * 0.5 + TAU * idx / 6.0
            sources.append(
                WaveSource(
                    f"hex_antinode_{idx:02d}",
                    "first_ring_antinode",
                    angle,
        ...
        for idx in range(12):
            ...
            sources.append(
                WaveSource(
                    f"outer_wave_{idx:02d}",
                    "outer_wave_support",
                    angle,
                    self.outer_source_radius,
                    0.54,
                    300.0 * s,
                    6,
                    0.29 * idx + 0.37,
    ...
    def _make_construction_circles(self) -> list[ConstructionCircle]:
        circles = [
            ConstructionCircle(
                ...
                self.cy,
                self.construction_radius,
                "center construction circle, radius tuned to first-ring spacing",
            )
        ]
        for idx in range(6):
            angle = -math.pi * 0.5 + TAU * idx / 6.0
            x = self.cx + self.construction_radius * math.cos(angle)
            y = self.cy + self.construction_radius * math.sin(angle)
            circles.append(
                ConstructionCircle(
                    f"hex_antinode_{idx:02d}_R1",
                    f"hex_antinode_{idx:02d}",
                    x,
                    y,
                    self.construction_radius,
                    "first-ring construction circle at hexagonal antinode position",
                )
            )
        return circles
    ...
    def _make_regions(self) -> tuple[CellRegion, list[CellRegion], list[CellRegion], list[CellRegion]]:
        center = self.construction_circles[0]
        ring = self.construction_circles[1:]
        center_mask = self.circle_masks[center.circle_id]
        center_core = self._solid_circle(center.x, center.y, center.radius_px * 0.25, soften=1.4 * self.scale)
        inner_bound = self._radial_band(center.radius_px * 0.10, center.radius_px * 1.12, 44.0 * self.scale)
        outer_bound = self._radial_band(center.radius_px * 0.80, center.radius_px * 1.95, 54.0 * self.scale)
        triple_bound = self._radial_band(center.radius_px * 0.30, center.radius_px * 1.04, 36.0 * self.scale)
        ...
        origin = CellRegion(
            ...
            (center.circle_id,),
            (center.source_id,),
            "central origin cell from the center construction circle at 0.25R",
            DEEP_TEAL,
            SOFT_GOLD,
            origin_mask,
        ...
        negatives: list[CellRegion] = []
        for idx, circle in enumerate(ring):
            circle_mask = self.circle_masks[circle.circle_id]
            lens = mask_subtract(mask_intersection(center_mask, circle_mask, inner_bound, self.field_boundary), center_core)
            lens = cv2.GaussianBlur(lens, (0, 0), 1.0 * self.scale)
            lenses.append(
                CellRegion(
                    f"two_circle_lens_{idx:02d}",
                    "two_circle_lens",
                    (center.circle_id, circle.circle_id),
                    (center.source_id, circle.source_id),
                    "intersection of center construction circle and adjacent antinode construction circle, central core removed",
                    WARM_CREAM if idx % 2 == 0 else ICE_BLUE,
                    INK,
                    lens,
                    mask_edge(lens, kernel=max(5, round(7 * self.scale) | 1), blur=2.0 * self.scale),
                )
            )

        for idx, circle in enumerate(ring):
            nxt = ring[(idx + 1) % len(ring)]
            c0 = self.circle_masks[circle.circle_id]
            c1 = self.circle_masks[nxt.circle_id]
            triple = mask_subtract(mask_intersection(center_mask, c0, c1, triple_bound, self.field_boundary), center_core)
            triple = cv2.morphologyEx(triple, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))
            triple = cv2.GaussianBlur(triple, (0, 0), 1.0 * self.scale)
            triples.append(
                CellRegion(
                    f"three_circle_cell_{idx:02d}",
                    "three_circle_trigon_like_cell",
                    (center.circle_id, circle.circle_id, nxt.circle_id),
                    (center.source_id, circle.source_id, nxt.source_id),
                    "three-circle intersection of center and adjacent first-ring construction circles, central core removed",
                    TEAL,
                    SOFT_GOLD,
                    triple,
                    mask_edge(triple, kernel=max(5, round(7 * self.scale) | 1), blur=2.2 * self.scale),
                )
            )

            negative = mask_subtract(mask_intersection(c0, c1, outer_bound, self.field_boundary), center_mask)
            negative = cv2.morphologyEx(negative, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))
            negative = cv2.GaussianBlur(negative, (0, 0), 1.2 * self.scale)
            negatives.append(
                CellRegion(
                    f"negative_space_trigon_{idx:02d}",
                    "negative_space_trigon_like_cell",
                    (circle.circle_id, nxt.circle_id, center.circle_id),
                    (circle.source_id, nxt.source_id, center.source_id),
                    "adjacent first-ring construction-circle intersection minus the center construction circle",
                    INK,
                    WARM_CREAM,
                    negative,
                    mask_edge(negative, kernel=max(5, round(9 * self.scale) | 1), blur=2.8 * self.scale),
                )
            )

        return origin, lenses, triples, negatives
    ...
    def render_frame(self, t: float, *, output_size: bool = True, mode: str = "beauty", labels: bool = False) -> tuple[np.ndarray, dict[str, object]]:
        ...
        if mode == "raw":
            ...
            if labels:
                draw_text(frame, "2 construction circles at source/antinode positions", (32, 48), 0.46)
        elif mode == "lenses":
...
def make_debug_sheet(renderer: PrimitiveEmergenceRenderer) -> Path:
    ...
    views = [
        ...
        ("construction circles", "construction"),
        ("2-circle lens cells", "lenses"),
    ...
    draw_text(sheet, "debug views: raw field -> source circles -> 2-circle cells -> 3-circle/negative cells -> beauty", (28, sheet.shape[0] - 24), 0.42)
...
def write_manifest(
    ...
    manifest = {
        ...
        "renderer": "scripts/abstract_cymatic_primitive_emergence_v001.py",
        "renderer_sha256": sha256(ROOT / "scripts" / "abstract_cymatic_primitive_emergence_v001.py"),
        ...
        "field_configuration": {
            "source_model": "procedural scalar wave emitters",
        ...
        "construction_circle_parameters": {
            ...
            "arrangement": "center plus six first-ring antinode construction circles in a hexagonal overlapping-circle layout",
            "circles": [
                {
        ...
        "selected_cell_logic": {
            ...
            "negative_space_trigons": [region.generation_rule for region in renderer.negative_regions],
            "cell_counts": {
                "origin": 1,
                "two_circle_lens": len(renderer.lens_regions),
                "three_circle_trigon_like_cell": len(renderer.triple_regions),
                "negative_space_trigon_like_cell": len(renderer.negative_regions),
            },
        },
...
def write_readme(renderer: PrimitiveEmergenceRenderer, loop_verification: dict[str, object], rendered: bool) -> Path:
    ...
    lines = [
        ...
        "## Purpose",
        "",
        "This packet explores circle, crescent/lens, and trigon-like geometry as abstract wave-derived construction. It uses no Austin artworks, no Austin motifs, no SVG assets, and no cultural-design claim.",
        "",
        "The primary peak forms are not wave-amplitude threshold blobs. They are arc-bounded cells created from overlapping construction circles placed at the procedural source/antinode positions. The wave field provides water atmosphere, drift, gather/release, and interior breathing.",
        "",
        "## Sequence",
        "",
        "- 0-8s: raw overlapping ripple/interference field.",
        "- 8-21s: construction circles become implied around source/antinode positions.",
        "- 21-32s: 2-circle lens/crescent cells emerge from center/ring circle intersections.",
        "- 32-48s: 3-circle cells and adjacent-ring negative-space trigon-like cells become strongest.",
        "- 48-60s: geometry releases back into the initial raw cymatic field and closes the loop.",
        "",
        "## Construction Logic",
        "",
        f"- Construction circles: `{len(renderer.construction_circles)}` total, center plus six first-ring antinode circles.",
        f"- Construction radius on work canvas: `{renderer.construction_radius:.3f}px`.",
        f"- 2-circle lens cells: `{len(renderer.lens_regions)}` center/ring intersections.",
        f"- 3-circle cells: `{len(renderer.triple_regions)}` center/ring/ring intersections.",
        f"- Negative-space trigon-like cells: `{len(renderer.negative_regions)}` adjacent-ring intersections minus the center circle.",
        "",
        "## Loop Verification",
        "",
        ...
        "- `stills/`: initial raw field, construction implied, lens emergence, trigon cells, peak beauty, return loop point.",
        f"- `debug/{PROJECT}_debug_views_sheet.png`: raw field, construction circles, 2-circle lens cells, 3-circle/negative-space cells, final beauty.",
    ...
    if seam_ok and raw_ok:
        lines.append(
            "v001 passes as a standalone abstract cymatic primitive-emergence study. The circle, crescent/lens, and trigon-like reads are visibly tied to overlapping construction circles rather than detached icons. The field still feels watery because the cells breathe over the interference layer instead of replacing it with a static diagram."
        )
    else:
        lines.append(
            "v001 has the intended construction-circle logic, but the loop check did not fully pass. Treat this as an internal draft until the loop seam is corrected."
        )
    lines.extend(
        [
            "",
            "Residual risk: the peak is intentionally more geometric than the earlier abstract cymatic composition. If it is pushed brighter or held longer, it could start reading as a diagram. In the current timing, the water field and opacity breathing keep it in the intended abstract wave-study lane.",
            "",
