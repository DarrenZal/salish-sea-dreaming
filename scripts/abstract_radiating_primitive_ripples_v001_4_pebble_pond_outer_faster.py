...
v001.4 — Pravin refinement 2026-05-24: pebble-in-pond. The outer (crescent↔trigon)
gap expands FASTER than the inner (circle↔crescent) gap as ripples radiate.
...
PROJECT = "abstract_radiating_primitive_ripples_v001_4_pebble_pond_outer_faster"
OUT_DIR = (
    ROOT
...
def render_ripple_system(canvas: np.ndarray, system: RippleSystem, frame_index: int, n_frames: int) -> None:
    ...
    for layer_index, (delay, alpha_scale, radius_scale, angle_jitter) in enumerate(layers):
        ...
        for k in range(system.count):
            ...
            # Pravin refinement 2026-05-24: pebble-in-pond — the outer ring
            # (crescent↔trigon) expands FASTER than the inner ring (origin↔crescent).
            # Linear proportional (v001.2/v001.3) made both grow at same rate, so the
            # outer:inner ratio stayed fixed. Here we add a super-linear term so the
            # ratio itself grows over the lifecycle: outer gap is ~18% of inner at
            # small radii, climbing to ~70% of inner at max radius.
            #
            # max() preserves close-spacing at very small radii (avoids overlap).
            TRI_GAP_BASE = 0.15        # ratio at very small radius
            TRI_GAP_EXTRA = 0.55       # additional ratio gained at max radius
            ripple_progress = min(1.0, radius_here / max_radius)
            prop_dynamic = TRI_GAP_BASE + TRI_GAP_EXTRA * ripple_progress
            tri_gap = max(
                size * 0.82 + trigon_size_base * 0.33,
                radius_here * prop_dynamic,
            )
