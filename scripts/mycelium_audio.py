...
# energy ratio 0-1) from an OSC In CHOP (auto-created, port 7000) and
# drives:
#   - pulse brightness  (volume 0.15 baseline -> 5.65 peak)
#   - glow size         (volume 5 baseline -> 45 peak)
#   - bright-pulse hue  (energy: low=teal/green, high=luminous cyan)
#
# Pulse motion speed is deliberately CONSTANT (slow diagonal drift).
# Audio-reactive motion caused "flashing" — position teleported when
# volume changed because rate*time with variable rate isn't continuous.
# Instead, audio drives visibility/intensity; motion is its own slow
# carrier wave regardless of what's playing.
#
# Aesthetic brief (Prav 2026-04-23): carrier wave memetics, stillness
# rewarded, depth over flash. Tuned live via TD MCP 2026-04-23.
#
# Signal chain:
#   audio_osc  ->  audio_lag  ->  audio_vals (rename -> 'volume','energy')
#                 (1.2s smoothing kills sudden jumps)
#
#   network_noise -> veins -> veins_boost --+---> dim_veins --+
#                                           |                 |
#   pulse_noise -> pulse_sharp --> pulsing--+                 |
#                                     |                       v
#                                     +--> pulse_boost --+   combined ---+
#                                                        |                |
#                                                        +-> bright_pulse-+
#                                                        |         |
#                                                        +-> glow--+
#                                                                  |
#                                                backdrop -------->+--> final_comp --> output
#                                                                  (with_glow OVER backdrop)
#
# Safe to run standalone: non-destructive coexistence with any
# pre-existing '/project1/salish' container from the original
# mycelium.py. Only touches '/project1/salish_audio'.

project = op('/project1')

# --- OSC In + Lag + Rename CHOP chain -------------------------------------
...
if audio_lag is None:
    audio_lag = project.create(lagCHOP, 'audio_lag')
    audio_lag.nodeX = -500
    audio_lag.nodeY = 300
audio_lag.par.lag1 = 1.2   # rise time (seconds)
audio_lag.par.lag2 = 1.2   # fall time
# wire (idempotent)
if not audio_lag.inputs or audio_lag.inputs[0] is not audio_osc:
    for i in range(len(audio_lag.inputConnectors)):
        try: audio_lag.inputConnectors[i].disconnect()
        except Exception: pass
    audio_lag.inputConnectors[0].connect(audio_osc)
...
if not audio_vals.inputs or audio_vals.inputs[0] is not audio_lag:
    for i in range(len(audio_vals.inputConnectors)):
        try: audio_vals.inputConnectors[i].disconnect()
        except Exception: pass
    audio_vals.inputConnectors[0].connect(audio_lag)

# Expression fragments. chan() returns None if the channel hasn't
# appeared yet — guard so silence before the first OSC message
# doesn't crash.
...
# Constant slow drift — carrier wave, not audio-driven velocity
...
# Audio-reactive brightness: near-black at silence, bright swell at peak
pulse_boost.par.brightness1.expr = f'0.15 + ({VOL}) * 5.5'
pulse_boost.par.gamma1 = 0.7
pulse_boost.nodeX = 600; pulse_boost.nodeY = -100

# === Colors ==============================================================
# Dim baseline mycelium — always visible teal, the "stillness" surface
...
# Bright-pulse hue — energy shifts teal-green -> luminous cyan
...
# Deep near-black backdrop — hint of teal, fully opaque.
# Without this, transparent pulse regions show TD's checkerboard.
...
final_comp.par.operand = 'over'  # with_glow OVER backdrop
...
print("mycelium_audio — live-tuned 2026-04-23 via TD MCP with Darren")
print(f"  OSC In   : {audio_osc.path}  port={audio_osc.par.port.eval()}")
print(f"  Lag      : {audio_lag.path}  (1.2s smoothing)")
print(f"  Rename   : {audio_vals.path}  -> 'volume', 'energy'")
