...
v001.3 — Austin feedback 2026-05-24 (more aggressive variant, TRI_GAP_PROPORTION=0.45):
proportional expansion of crescent↔trigon gap as radial position grows.
...
PROJECT = "abstract_radiating_primitive_ripples_v001_3_austin_proportional_aggressive"
OUT_DIR = (
    ROOT
...
def render_ripple_system(canvas: np.ndarray, system: RippleSystem, frame_index: int, n_frames: int) -> None:
    ...
    for layer_index, (delay, alpha_scale, radius_scale, angle_jitter) in enumerate(layers):
        ...
        for k in range(system.count):
            ...
            TRI_GAP_PROPORTION = 0.45  # tunable (v001.3 more aggressive variant)
            tri_gap = max(
