#!/usr/bin/env python3
"""
Anchor graph spine v003 cymatic loop.
...
-> Raven anchor collapse -> cymatic regather -> Nature Cosmic Sun return.
...
KEY_STILL_TIMES = (
    ...
    ("raven_hold", 48.0),
    ("raven_collapse", 58.0),
    ("cymatic_regather", 65.0),
    ("nature_return", 70.5),
)

CONTACT_TIMES = (0.0, 6.0, 10.5, 12.5, 16.0, 20.0, 24.0, 28.0, 32.0, 36.0, 42.0, 48.0, 56.0, 64.0, 68.5, 71.5)
...
def spine_coherence(time_seconds: float) -> float:
    ...
    if time_seconds < 54.0:
        return 1.0
    if time_seconds < 62.0:
        return 1.0 - 0.74 * smoothstep01((time_seconds - 54.0) / 8.0)
    if time_seconds < 68.0:
        return 0.26 + 0.74 * smoothstep01((time_seconds - 62.0) / 6.0)
    return 1.0
...
def nature_opacity(time_seconds: float) -> float:
    ...
    ret = smoothstep01((time_seconds - 68.0) / 3.2)
    if time_seconds < 8.0:
        return 0.0
    if time_seconds < 32.0:
...
def raven_opacity(time_seconds: float) -> float:
    ...
    if time_seconds >= 44.0:
        in_level = 1.0
    out_level = 1.0 - smoothstep01((time_seconds - 58.0) / 4.0)
...
def wave_levels(time_seconds: float) -> dict[str, float]:
    ...
    return_level = smoothstep01((time_seconds - 54.0) / 8.0) * (1.0 - 0.35 * smoothstep01((time_seconds - 68.0) / 4.0))
    gather = smoothstep01((time_seconds - 0.5) / 7.5)
    regather = smoothstep01((time_seconds - 62.0) / 6.0)
    return {
        "water": clamp01(0.94 - 0.34 * art_dominance + 0.18 * return_level),
        "positive": clamp01(0.88 * (1.0 - 0.82 * art_dominance) + 0.20 * return_level + 0.10 * regather),
        "negative": clamp01(0.78 * (1.0 - 0.50 * art_dominance) + 0.12 * return_level + 0.08 * regather),
        "line": clamp01(0.56 + 0.18 * max(gather, regather) - 0.12 * art_dominance + 0.20 * return_level),
    }
...
def nature_full_hold_mask(nature: ArtworkLayer) -> np.ndarray:
    ...
    if NATURE_FULL_HOLD_MASK is None:
        x0, y0, x1, y1 = [int(v) for v in nature.info["bbox_px"]]
        NATURE_FULL_HOLD_MASK = soft_artwork_alpha(nature.alpha, (x0, y0, x1, y1), feather_px=34.0)
...
def nature_presentation_mask(nature: ArtworkLayer, mask: np.ndarray) -> np.ndarray:
    ...
    black_disc = (DIST_FROM_ANCHOR <= ANCHOR_RADIUS_PX * 0.985) & (luminance < 34.0) & (nature.alpha > 8)
    alpha[black_disc] *= 0.86
...
def raven_spatial_reveal_mask(time_seconds: float) -> np.ndarray:
    ...
    if time_seconds < 62.0:
        collapse = smoothstep01((time_seconds - 54.0) / 4.0)
        radius = 1320.0 * (1.0 - collapse) + (ANCHOR_RADIUS_PX + 22.0) * collapse
...
def portal_alignment_alpha(time_seconds: float) -> float:
    ...
    return clamp01(max(gather, regather))
...
def render_frame(
    ...
    *,
    ...
    if debug:
        base.draw_text(frame, f"anchor_graph_spine_v003_cymatic_loop t={time_seconds:05.2f}", (42, 56), LABEL, scale=0.50)
    return frame, info
...
def save_key_stills(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, str]:
    ...
    for key, time_seconds in KEY_STILL_TIMES:
        ...
        path = STILLS_DIR / f"anchor_graph_spine_v003_cymatic_loop_{key}.png"
        cv2.imwrite(str(path), frame)
        outputs[f"{key}_still"] = str(path.relative_to(OUT_DIR))
    peak_frame, _info = render_frame(12.8, nature, raven)
    peak_path = PEAK_DIR / "anchor_graph_spine_v003_cymatic_loop_nature_portal_peak.png"
...
def make_contact_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    path = OUT_DIR / "anchor_graph_spine_v003_cymatic_loop_contact_sheet.png"
    cv2.imwrite(str(path), sheet)
    return path


def make_v002_v003_comparison_sheet(contact_path: Path) -> Path:
    v002_path = (
        ROOT
        / "track2-deterministic"
        / "morph_outputs_INTERNAL"
        / "anchor_graph_spine_v002_2026-05-21"
        / "anchor_graph_spine_v002_contact_sheet.png"
    )
    v002 = cv2.imread(str(v002_path), cv2.IMREAD_COLOR)
    v003 = cv2.imread(str(contact_path), cv2.IMREAD_COLOR)
    if v002 is None:
        raise RuntimeError(f"could not read v002 contact sheet: {v002_path}")
    if v003 is None:
        raise RuntimeError(f"could not read v003 contact sheet: {contact_path}")
    target_w = max(v002.shape[1], v003.shape[1])
    v002 = cv2.resize(v002, (target_w, round(v002.shape[0] * target_w / v002.shape[1])), interpolation=cv2.INTER_AREA)
    v003 = cv2.resize(v003, (target_w, round(v003.shape[0] * target_w / v003.shape[1])), interpolation=cv2.INTER_AREA)
    label_h = 56
    bar1 = np.full((label_h, target_w, 3), rgb_to_bgr(DEEP_GROUND), dtype=np.uint8)
    bar2 = bar1.copy()
    base.draw_text(bar1, "v002: 60s lead spine, aperture-cropped Nature hold", (28, 38), LABEL, scale=0.48)
    base.draw_text(bar2, "v003: 72s cymatic loop, full Nature hold, stable Raven hold, return gather", (28, 38), LABEL, scale=0.48)
    sheet = cv2.vconcat([bar1, v002, bar2, v003])
    path = OUT_DIR / "anchor_graph_spine_v002_v003_comparison_sheet.png"
    cv2.imwrite(str(path), sheet)
    return path
...
def make_timeline_panel(width: int, height: int) -> np.ndarray:
    ...
    segments = (
        ...
        ("cymatic regather", 62.0, 68.0, MUTED_TEAL),
...
def make_debug_sheet(nature: ArtworkLayer, raven: ArtworkLayer) -> Path:
    ...
    panel_specs = (
        ...
        ("full Nature hold: whole source + residue", 19.0),
        ("shared anchor: sun-disc radius aligned", 28.0),
        ("Raven reveal: expansion from shared disc", 38.0),
        ("stable Raven hold: frozen transform", 48.0),
        ("Raven collapse: source recedes to anchor", 58.0),
        ("cymatic regather: C8 target rebuild", 65.0),
        ("Nature return: portal to whole source", 70.5),
    )
    panels: list[np.ndarray] = []
    for label, time_seconds in panel_specs:
        frame, _info = render_frame(time_seconds, nature, raven)
        nature_mask, _edge = nature_mask_at(time_seconds, nature)
        _raven_bgr, raven_mask, _raven_info = raven_spatial_frame_at(time_seconds) if 32.0 <= time_seconds < 62.0 else (
            raven.bgr,
    ...
    middle = cv2.hconcat(panels[3:6])
    bottom = cv2.hconcat(panels[6:9])

    mask_panel = np.full((360, 1920, 3), rgb_to_bgr(DEEP_GROUND), dtype=np.uint8)
    n_mask, _edge = nature_mask_at(12.5, nature)
    n_return_mask, _edge = nature_mask_at(70.5, nature)
    _raven_bgr, r_mask, _raven_info = raven_spatial_frame_at(38.0)
    _raven_bgr, r_collapse_mask, _raven_info = raven_spatial_frame_at(58.0)
    n_mask_small = cv2.resize(n_mask, (1920, 360), interpolation=cv2.INTER_AREA)
    n_return_small = cv2.resize(n_return_mask, (1920, 360), interpolation=cv2.INTER_AREA)
    ...
    r_collapse_small = cv2.resize(r_collapse_mask, (1920, 360), interpolation=cv2.INTER_AREA)
    ...
    wave_v002.blend_mask(mask_panel, r_collapse_small, PORTAL_ORANGE, 0.20)
    wave_v002.blend_mask(mask_panel, n_return_small, SOFT_GOLD, 0.24)
    ...
    base.draw_text(mask_panel, "loop diagnostic masks: Nature portal/return + metadata-only shared anchor + Raven reveal/collapse", (42, 52), LABEL, scale=0.46)
    ...
    sheet = cv2.vconcat([top, middle, bottom, mask_panel, timeline])
    path = DEBUG_DIR / "anchor_graph_spine_v003_cymatic_loop_loop_return_diagnostic.png"
    cv2.imwrite(str(path), sheet)
    return path


def render_clip(nature: ArtworkLayer, raven: ArtworkLayer) -> dict[str, object]:
    output_path = OUT_DIR / "anchor_graph_spine_v003_cymatic_loop.mp4"
    ...
    try:
        for frame_idx in range(N_FRAMES):
            ...
            if (frame_idx + 1) % FPS == 0:
                print(f"  anchor_graph_spine_v003_cymatic_loop.mp4 {frame_idx + 1}/{N_FRAMES}", flush=True)
    finally:
        writer.close()
    if peak_info is None:
        _frame, peak_info = render_frame(12.8, nature, raven, collect_info=True)
    return {
        "filename": "anchor_graph_spine_v003_cymatic_loop.mp4",
...
def write_manifest(
    ...
    manifest = {
        "project": "anchor_graph_spine_v003_cymatic_loop",
        ...
        "renderer": "scripts/anchor_graph_spine_v003_cymatic_loop.py",
        "renderer_sha256": sha256(ROOT / "scripts" / "anchor_graph_spine_v003_cymatic_loop.py"),
        ...
        "spatial_node_integration": {
            ...
            "integration_method": "The clean-frame Raven spatial frames are composited through source-alpha and source-aligned reveal/collapse masks. The reveal is anchor-locked to the shared 276 px radius; the hold uses deterministic frame interpolation over a narrow monotonic frame range with the anchor transform frozen at the hold entrance to avoid the v002 wobble.",
            "anchor_stabilization": {
                ...
                "hold_transform_policy": "freeze projection transform at spatial-node norm 0.82; continue only a small monotonic interpolated frame sample from norm 0.82 to 0.865 during hold",
        ...
        "opacity_and_blend_settings": {
            ...
            "nature_dark_disc_note": "v003 rasterizes Nature with CairoSVG so the full authored yellow center is preserved; the multiplier remains active for genuinely dark source pixels, but the v002 black center was an ImageMagick fill-none artifact rather than a source-faithful disc.",
            "nature_full_hold_mask": "whole source alpha with 34 px presentation edge feather around the raster bounds",
        ...
        "timings": [
            ...
            },
            {
                ...
                "notes": "Transitions out of the portal aperture into the whole source artwork, including the authored yellow sun disc, outward rays, and cream background. The whole-source raster uses a soft presentation edge to avoid a hard card read.",
            },
            {
            ...
            },
            {
                ...
                "anchors_used": ["animal_bird_raven_sun.central_sun_disc"],
                "transition_method": "Raven clean-frame source-alpha mask closes toward the shared sun-disc radius while cymatic field levels rise.",
            },
            {
                "segment": "cymatic field regathers into Nature alignment",
        ...
        "tested_edges": [
            {
                "edge_id": "wave_to_nature_cosmic_sun_spine_v003_cymatic_loop",
                "edge_type": "wave_to_artwork",
                ...
                "artifact_path": str((OUT_DIR / "anchor_graph_spine_v003_cymatic_loop.mp4").relative_to(ROOT)),
                "caveats": [
            ...
            {
                "edge_id": "nature_cosmic_sun_to_raven_sun_spine_v003_cymatic_loop",
                "edge_type": "artwork_to_artwork",
            ...
            {
                "edge_id": "raven_sun_to_cymatic_to_nature_return_spine_v003",
                "edge_type": "artwork_to_wave_atmosphere",
                "status": "internal_composition_pass",
        ...
        "deliverables": {
            ...
            "v002_v003_comparison_sheet": str(comparison_path.relative_to(OUT_DIR)),
            "loop_return_diagnostic_sheet": str(debug_path.relative_to(OUT_DIR)),
            "source_provenance_note": str(provenance_note.relative_to(OUT_DIR)),
            "readme": "README.md",
            **stills,
        },
    }
    path = OUT_DIR / "anchor_graph_spine_v003_cymatic_loop_manifest.json"
...
def write_readme(nature: ArtworkLayer, raven: ArtworkLayer, clip_summary: dict[str, object]) -> Path:
    lines = [
        ...
        "## Purpose",
        "",
        "This packet tests the current canonical installation primitive: artwork -> anchor collapse -> cymatic field -> target gather -> whole artwork reveal. It extends the v002 lead spine into a 72 second loop passage: cymatic/water atmosphere -> Nature Cosmic Sun portal -> full Nature hold -> shared sun-disc anchor -> Raven spatial reveal -> stable Raven spatial hold -> Raven collapse -> cymatic regather -> Nature return.",
        "",
        ...
        "- 8-14s: Nature Cosmic Sun opens through center/ray aperture.",
        "- 14-24s: full `Nature_Cosmic_Sun.svg` authored artwork resolves and holds, including the yellow sun disc, outward rays, and cream background.",
        "- 24-32s: Nature collapses toward the shared sun-disc anchor.",
        "- 32-44s: Raven Sun emerges through the shared sun-disc anchor using the clean-frame spatial treatment.",
        "- 44-54s: Raven spatial hold uses deterministic frame interpolation and a frozen hold transform to remove v002 shake.",
        "- 54-62s: Raven collapses through the sun-disc anchor back into cymatic/water field.",
        "- 62-68s: cymatic field regathers into the 8-source Nature alignment.",
        "- 68-72s: Nature portal return resolves into full Nature. This was chosen over ending at aperture-only because the loop test is validating whole-artwork reveal.",
        "",
        "## Deliverables",
        "",
        "- `anchor_graph_spine_v003_cymatic_loop.mp4`: 1920x1080, 24 fps, 72 seconds.",
        "- `anchor_graph_spine_v003_cymatic_loop_contact_sheet.png`: labeled timestamp sequence sheet.",
        "- `stills/`: initial cymatic, Nature portal, full Nature hold, shared anchor, Raven reveal, Raven hold, Raven collapse, cymatic regather, and Nature return.",
        "- `anchor_graph_spine_v002_v003_comparison_sheet.png`: v002/v003 comparison sheet.",
        "- `debug_stills/anchor_graph_spine_v003_cymatic_loop_loop_return_diagnostic.png`: loop/return diagnostic sheet.",
        "- `anchor_graph_spine_v003_cymatic_loop_manifest.json`: sources, hashes, anchors, timings, opacity/blend settings, Raven stabilization, and boundary flags.",
        "- `SOURCE_PROVENANCE.md`: local source provenance and boundary note.",
        "",
        "## Honest Verdict",
        "",
        "This is closer to a finished installation primitive than v002. The most important correction is that the Nature section now resolves into the full authored artwork rather than staying on the aperture-cropped black-disc motif. I also switched v003 Nature rasterization to CairoSVG because the v002 ImageMagick path filled SVG `fill:none` areas as black; with the source-faithful raster, the authored yellow center appears. The dark-disc presentation-alpha guard remains in code for genuine dark source pixels, but the visible v002 black center was a rasterization artifact.",
        "",
        "The Raven hold is intentionally calmer than the standalone spatial node. The reveal remains anchor-locked, then the hold freezes the anchor transform and samples a narrow monotonic clean-frame range with interpolation. This removes the small v002 wobble at the cost of less orbit drama; that is the right tradeoff for an installation spine.",
        "",
        "The return works structurally: Raven closes through the same center, the cymatic field reasserts itself, and the Nature target gathers back from the field. The roughest remaining edge is the full-source Nature rectangle management; the soft presentation edge prevents a hard card snap, but this is still the area to review on a wall-scale display.",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render anchor graph spine v003 cymatic loop.")
    ...
    comparison_path = make_v002_v003_comparison_sheet(contact_path)
    ...
    if args.preview:
        ...
        clip_summary = {
            "filename": "anchor_graph_spine_v003_cymatic_loop.mp4",
            "mp4": "anchor_graph_spine_v003_cymatic_loop.mp4",
            "duration_seconds": DURATION_SECONDS,
            "fps": FPS,
            "frame_count": N_FRAMES,
            "peak_time_seconds": 12.8,
        ...
        print("Rendering anchor_graph_spine_v003_cymatic_loop.mp4", flush=True)
