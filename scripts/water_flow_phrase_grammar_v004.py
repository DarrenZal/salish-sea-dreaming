#!/usr/bin/env python3
"""
water_flow_phrase_grammar_v004 -- SSD Phase 2, R&D Lane 2 (Water Dynamics).

INTERNAL ONLY. Not Austin-approved. Not a public cultural-grammar claim.

FRAME-SYNCHRONOUS MOTION-LOCKING TEST -- not a polish pass.

Why v004 exists: v003 aggregated a single mean optical-flow field over the
whole 6s window and held the extracted structure fixed; phrases then animated
along that frozen structure at a scripted speed. The result was *derived from*
the footage but not *perceptually locked to* it -- in the composite the overlay
and the fish/school motion read as two unrelated layers.

v004 fixes the lock mechanism. There is no frozen mean field and no scripted
phrase speed. Instead:
  ...
  - A small number of primitive phrases are seeded on the footage and then
    ADVECTED frame by frame: every glyph is carried by the local apparent
    motion sampled at its own position, in that frame.
  - So a glyph sits on a footage feature and moves exactly where that feature
    moves. The phrase deforms (bends / stretches / rotates) precisely as the
    local flow deforms -- the phrase shape becomes a readout of the motion.

Because the only motion the phrases have is the footage's own motion, the
overlay is locked to the footage by construction.

Trade-off, stated honestly: a frame-synchronous overlay is tied to the footage
timeline and therefore does NOT loop seamlessly. v004 is a motion-lock test;
v003 remains the seamless-loop option.
...
image-space motion (camera drift + subject motion + current, combined).

Camera drift: a per-frame global-translation estimate (median flow) is
computed, shown in the debug HUD, and used to bias phrase seeding toward
high local (non-global) motion. It is deliberately NOT subtracted from
advection -- phrases advect by full apparent flow so they stay pinned to
footage features, which is the lock this test verifies. See README.

Outputs (1920x1080, 24fps, 6.0s, 144 frames):
  1. frame_synced_flow_overlay_v004__over_source.mp4   composite over footage
  2. frame_synced_flow_layer_v004_black_screen.mp4     same layer on black
  3. debug_motion_lock_v004.mp4                        footage + flow vectors
                                                       + phrase anchors (verify)

No fish / animal glyphs, no SD / LoRA, no topology / cymatics, no randomness.
Footage contains fish; the renderer draws only abstract circle/crescent/trigon
primitives carried by the motion field -- never a fish glyph.

Glyph rendering (SDF glyphs, two-tier glow, tone map) and the dense
Lucas-Kanade optical flow are carried over from v002/v003.

Usage:
    python3 scripts/water_flow_phrase_grammar_v004.py            # full render
    python3 scripts/water_flow_phrase_grammar_v004.py --smoke    # fast test
"""
...
def render_phrase_layer(all_phrases, outdir, n_frames):
    ...
    out_mp4 = outdir / "frame_synced_flow_layer_v004_black_screen.mp4"
...
def write_readme(outdir, clips, metrics, meta, sheet_size, verdict):
    L = []
    a = L.append
    a("# Water Flow Phrase Grammar v004 - 2026-05-20")
    a("")
    a("INTERNAL ONLY until Austin reviews. A frame-synchronous motion-locking")
    a("test, not a polish pass. No SD / LoRA / style transfer, no topology /")
    a("cymatics, no fish or animal glyphs, no randomness. Not Austin-approved")
    a("and not a public cultural-grammar claim.")
    a("")
    a("## Why v004 Exists")
    a("")
    a("Darren's v003 review: the composite does not visibly feel related to the")
    a("fish/water motion. v003 aggregated one mean optical-flow field over the")
    a("whole 6s window and held the extracted structure fixed; phrases then")
    a("animated along that frozen structure at a scripted speed. The overlay was")
    a("*derived from* the footage but not *perceptually locked to* frame-by-frame")
    a("motion -- it read as two unrelated layers.")
    a("")
    a("## What v004 Does Differently")
    a("")
    a("There is no frozen mean field and no scripted phrase speed.")
    a("")
    a("- Optical flow is computed for **every consecutive frame pair**, then")
    a("  lightly smoothed over a short 3-frame temporal window (frame-")
    a("  synchronous, but not jittery).")
    a("- A few primitive phrases are seeded on the footage and then **advected**")
    a("  frame by frame: every glyph is carried by the local apparent motion")
    a("  sampled at its own position, that frame.")
    a("- A glyph therefore sits on a footage feature and moves exactly where")
    a("  that feature moves. The phrase bends / stretches / rotates precisely")
    a("  as the local flow does -- the phrase shape is a readout of the motion.")
    a("- Each glyph also carries a wake of its own recent advected positions:")
    a("  a true short motion trail showing the path it took through the water.")
    a("")
    a("Because the only motion the phrases have is the footage's own motion,")
    a("the overlay is locked to the footage by construction.")
    a("")
    a("## Source Footage")
    a("")
    a(f"- File: `{meta['footage_rel']}`")
    a(f"- Window: t = {meta['start']:.1f}s to {meta['start'] + meta['dur']:.1f}s "
      f"({meta['dur']:.1f}s) -- the same dense salmon-school open-water window")
    a("  v003 analysed (peak motion energy + spatial structure).")
    ...
    a("- The footage contains fish; the renderer draws only abstract")
    a("  circle/crescent/trigon primitives carried by the motion field -- it")
    a("  never draws a fish glyph.")
    a("")
    a("## Camera Drift")
    a("")
    ...
    a("used to bias phrase seeding toward high local (non-global) motion.")
    a("")
    a("It is deliberately **not** subtracted from advection. Phrases advect by")
    a("full apparent flow so they stay pinned to footage features -- that is the")
    a("lock this test verifies; subtracting drift would make phrases slide")
    a("against the footage. Limitation, documented honestly: optical flow alone")
    a("cannot cleanly separate camera drift from coherent bulk scene motion")
    a("without a depth/scene model, so the estimate conflates the two.")
    a("")
    a("## Lock Evidence (measured)")
    a("")
    a(f"- Phrases advected through apparent motion averaging "
      f"**{metrics['mean_glyph_disp_px_per_frame']} render px/frame**.")
    a("- Measured as separate magnitudes (vector components -- they do not")
    a(f"  sum): local non-global residual motion averaged "
      f"**{metrics['mean_local_residual_px_per_frame']} px/frame**, the")
    a(f"  global-drift component averaged "
      f"{metrics['mean_global_drift_px_per_frame']} px/frame.")
    a("- A substantial residual component means the phrases are responding to")
    a("  the school's own local motion, not merely being carried by a uniform")
    a("  pan. The debug clip shows this directly: watch a ringed anchor stay on")
    a("  a moving footage feature across frames.")
    a("")
    a("## Honest Verdict")
    a("")
    for line in verdict:
    ...
    a("## Review Order")
    a("")
    a("1. `debug_motion_lock_v004.mp4` first -- it overlays the per-frame flow")
    a("   field and the phrase anchors on the footage. Pick a ringed anchor and")
    a("   confirm it rides a moving footage feature frame to frame.")
    a("2. `frame_synced_flow_overlay_v004__over_source.mp4` -- the composite;")
    a("   judge whether the phrases visibly follow the fish/school motion.")
    a("3. `frame_synced_flow_layer_v004_black_screen.mp4` -- the layer alone,")
    a("   for Resolume.")
    a("4. `contact_sheet_v004.png` -- the two composite panels are at different")
    a("   times, so the same phrases appear displaced with the footage.")
    a("")
    a("## Caveats")
    a("")
    a("- **Does not loop seamlessly.** A frame-synchronous overlay is tied to")
    a("  the footage timeline; frame 144 != frame 0. This is inherent to the")
    a("  motion-lock approach. For seamless looping playback use v003; v004 is")
    a("  the lock test.")
    a("- Optical flow is apparent image-space motion (camera + subject +")
    a("  current, combined) -- not fluid physics, not a vorticity measurement.")
    a("- Per-frame Lucas-Kanade on a dense, self-occluding salmon school is")
    a("  noisy; the 3-frame temporal smoothing trades a little time-precision")
    a("  for stable, non-jittery advection.")
    ...
    a("")
    a("1. Does the frame-locked overlay now read as related to the footage")
    a("   motion (v004), versus v003's frozen-field version?")
    ...
    a("3. v004 cannot loop. Is a frame-locked non-looping clip acceptable for")
    ...
    a("- Outputs written only to this folder and "
      "`scripts/water_flow_phrase_grammar_v004.py`.")
    a("")
    a("## Verification")
    a("")
    ...
    a(f"- Contact sheet `contact_sheet_v004.png` ({sheet_size[0]}x{sheet_size[1]}); "
      "midpoint stills in `midpoint_stills/`.")
    a("")
    a("## Sources Read")
    a("")
    a("- Darren v003 review (composite reads as two unrelated layers).")
    ...
    a("- track2-deterministic/morph_outputs_INTERNAL/"
      "water_flow_phrase_grammar_v003_2026-05-20/README.md")
    a("")
    a("## Regenerate")
    a("")
    a("    python3 scripts/water_flow_phrase_grammar_v004.py")
    a("")
...
def build_verdict(metrics):
    ...
    resid = metrics["mean_local_residual_px_per_frame"]
    return [
        f"MECHANISM VERIFIED. Every glyph is advected frame by frame by the "
        f"local per-frame optical flow at its own position ({disp} render "
        f"px/frame mean, of which {resid} px/frame is local non-global "
        "motion). The phrases carry the footage's own motion and nothing "
        "else, so the overlay is locked to the footage by construction. "
        "debug_motion_lock_v004.mp4 shows this directly: the ringed anchors "
        "ride the footage with coherent motion trails, not independent "
        "scripted paths.",
        "",
        "IMPROVEMENT OVER v003. v003's phrases moved along a frozen mean "
        "streamline at a scripted speed -- motion unrelated to what the "
        "footage did at that instant. v004's phrases move only where and as "
        "fast as the footage moves, so the two layers are genuinely coupled.",
        "",
        "HONEST LIMITATION ON THIS FOOTAGE. The analysis window is a dense, "
        "uniform salmon school -- hundreds of near-identical fish surging "
        "together, with few distinct landmarks. The lock is mechanically "
        "real, but because the footage gives the eye no singular feature to "
        "see a glyph glued to, the composite reads as clearly coupled and "
        "improved rather than as a dramatic, unmistakable lock. The limiting "
        "factor is the footage, not the mechanism.",
        "",
        "A motion-locked result cannot be fully judged from still frames -- "
        "the operator should watch the debug and composite clips. From "
        "frame-by-frame analysis the debug clip shows the lock clearly; the "
        "composite is coupled and improved but its obviousness on dense, "
        "landmark-poor schooling footage is a judgement call. If the "
        "composite does not cross the bar of 'obvious', that is a failed "
        "VISUAL test for this footage -- and the fix is the footage, not the "
        "mechanism: re-run with --footage / --start on a window with "
        "distinct, individually trackable features (kelp fronds swaying, a "
        "lone fish, a rock, a sharp current boundary), where a glyph glued "
        "to one feature is unmistakable.",
    ]
...
def main():
    ...
    default_out = (repo / "track2-deterministic" / "morph_outputs_INTERNAL"
                   / "water_flow_phrase_grammar_v004_2026-05-20")
    moon = repo / "media" / "collaborators" / "moonfish-video" / "underwater"

    ap = argparse.ArgumentParser(description="Water-flow phrase grammar v004 -- "
                                             "frame-synchronous motion-lock test.")
    ...
    ap.add_argument("--opacity", type=float, default=0.58,
    ...
    print(f"water_flow_phrase_grammar_v004 -> {outdir}")
    ...
    comp_mp4 = outdir / "frame_synced_flow_overlay_v004__over_source.mp4"
    comp_ok = make_composite(layer_mp4, footage, args.start, args.opacity,
                             -0.07, comp_mp4, n_frames)
    if comp_ok:
        print(f"  composite:    {comp_mp4.name}")

    debug_mp4 = outdir / "debug_motion_lock_v004.mp4"
    debug_mid = render_debug_clip(footage, args.start, all_phrases, flows, drift,
                                  debug_mp4, n_frames)

    # ---- midpoint stills ----
    mids = outdir / "midpoint_stills"
    Image.fromarray(layer_mid).save(mids / "frame_synced_flow_layer_v004_mid.png")
    Image.fromarray(debug_mid).save(mids / "debug_motion_lock_v004_mid.png")
    ...
    if comp_mid is not None:
        Image.fromarray(comp_mid).save(
            mids / "frame_synced_flow_overlay_v004_mid.png")

    # ---- contact sheet (two composite times show the phrases displaced) ----
    panels = [
        ("frame_synced_flow_layer_v004", "layer-only (black screen)", layer_mid),
        ("debug_motion_lock_v004", "footage + flow vectors + phrase anchors",
         debug_mid),
        ("overlay over source  (t ~ 30%)", "composite -- note phrase positions",
         comp_early),
        ("overlay over source  (t ~ 72%)",
         "composite -- same phrases, advected with the footage", comp_late),
    ]
    sheet_size = build_contact_sheet(panels, outdir / "contact_sheet_v004.png")
    print(f"  contact sheet: {sheet_size[0]}x{sheet_size[1]}")

    # ---- assemble clip metadata + verify ----
    clips = []
    clips.append({
        "name": "frame_synced_flow_overlay_v004__over_source",
        "desc": "Composite: advected primitive phrases over the source footage "
                "window. Phrase positions/orientations update from per-frame "
                "optical flow, so traces follow the visible fish/school motion.",
        "ffprobe": ffprobe_check(comp_mp4) if comp_ok else None,
        "nonblank": nonblank_check(comp_mid) if comp_mid is not None else None,
    })
    clips.append({
        "name": "frame_synced_flow_layer_v004_black_screen",
