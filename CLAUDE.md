...
> 🎯 **AGOG grant SUBMITTED (2026-06-12):** Full AGOG Open Call 2026 ("Climate Futures + Immersive Media") application **submitted via Submittable before the deadline**. Ask **$150,000** (right-sized below the $200K ceiling per Brandon's read of the competitive $1M pool); applicant = **Department of Bioregion** (US 501c3, EIN 85-4402939, Brandon Letsinger), Darren submitted via own account. **Reframed around per-place co-creation** (each of 8-10 Cascadia stops = a custom installation built with *local artists + local ecological data*; largest budget share $55K). Multi-agent adversarial review killed real fabrications: the invented "Map of Silence" audio (real = Matt Robertson's herring catch/spawn-data musical score), the coined term "Kwaxala," and an unsourced "15% commitment" metric. Voice pulled to "witnessing is rewarded" (ties Carol Anne's framework). Deck: 10 slides, real show photos + Briony Penn's Salish Sea panorama + live 48-node dreamworld. Packet: **`docs/grants/agog-opencall-2026/`** (UNTRACKED — not committed). Also this session: seeded 36 authored dreams (cloud 12→48), fixed home→dreamworld link to the `ongoing` window.
>
> ⏳ **Two post-submit follow-ups owed:** (1) populate the **new public repo `github.com/DarrenZal/SalishSeaDreaming`** (linked in the submission, currently EMPTY) with reviewer-safe content only (README + web prototype + public docs; NO internal/Coast-Salish/grant-internal material); (2) send **Brandon a PDF copy** of the application (he asked). Telegram session is broken (Telethon SQLite "too many values to unpack").
>
> ✅ **BCAC grant support + `.art` "About this project" page LIVE (2026-06-23/24, session `21dd2fa5`):** Pravin's recent grant asks were about a *new* grant — **BC Arts Council Media Artists (due Jun 24)**, not AGOG (already in). (1) Produced Darren's signed **Letter of Commitment** PDF ($3,250 allocation, modeled on Jason Kitikul's; `docs/grants/bcac-media-artists-2026/`, typed sig block) — operator sent it to Pravin. (2) **Deployed an "About this project" section live to `salishseadreaming.art`** — home intro blurb + "About this project" card + new **`/about/project`** canon page — using *Pravin's own* About-the-Artwork/Artist copy from his Canva site + his canonical credits. Mechanism: md in `ssd-v5:docs/about/about-the-artwork.md` wired via `_canon_dirs_map["project"]` + `index.html` edits; **staged on v6→promoted to v5**; backups `.bak-about-20260623-2346`; no regressions. Adversarial as-built check dropped a "tide/lunar/river drives the visuals" line (aspirational, from `podcast-source-document.md`, NOT as-built). Don't-name-KOI + herring-data=Shawn/Eve corrections honored. Signal note to Pravin sent. See `project_ssd_credit_attribution` (now holds Pravin's canonical credit list).
>
...
**Date:** 2026-06-24
**Status:** **AGOG grant submitted; BCAC LOC + `.art` About-page done (see banner).** Otherwise: between installations — IMPACT 2026 over (May 27–28, HR MacMillan Space Centre). 3090 R&D idle. Next target: **MOVE37XR Dome Theatre, Oct 2026** (then DEVCON ETH Mumbai, Nov 2026).
...
1. **AGOG post-submit (owed, tied to the live submission):** (a) populate **`github.com/DarrenZal/SalishSeaDreaming`** (public, linked in the grant, EMPTY) with reviewer-safe content only — clean README + `web/` prototype + public one-pager; **exclude** all internal/Coast-Salish/grant-internal docs. (b) Send **Brandon** a PDF of the application (Telegram broken → email or paste). (c) Optional: commit the `docs/grants/agog-opencall-2026/` packet to the repo. (d) Pravin may still answer the Austin-naming question (kept general).
...
2. **`_label_clusters` 10-line fix** — swap `openai_client` → `chat_client` in ssd-v5 `gallery_server.py:1603` (removes 401 log noise; restart 9004 per `reference_ssd_v5_prod_service_restart`).
3. **Optional 3090 disk free-up** (~200 GB) — candidates in `docs/space-center/3090-disk-archive-candidates.md`; deferred pending an external destination.
4. **Arshia follow-up** — positive post-show note (Signal, May 28); reconnect before MOVE37XR given the Autolume lineage.
5. **Cleanup (low pri):** delete `*.bak-2026-06-10` rollback files on poly ssd-v5 once consent-flip is confirmed stable for a few days; also the `.bak-about-2026062*` files once the About page is confirmed stable.
6. **Optional follow-ups (this session's tail):** commit the UNTRACKED `docs/grants/bcac-media-artists-2026/` + `docs/website/` to the repo; `/about/project` credits use Pravin's published wording (no Austin re-sign-off needed, confirm if desired); could add the about-copy to the "Ask the dream field" witness canon (currently page-only). Superseded drafts in `docs/website/` (the AGOG-derived `about-salish-sea-dreaming.md` + `signal-pravin-about-copy-2026-06-23.md`) can be deleted.
...
| `18395e7d` | 2026-06-10 | Post-IMPACT between-installations consolidation (7 phases) + live consent archival | **Tunneled into 3090** (now at Darren's house; updated `~/.ssh/config` `windows-desktop`→192.168.1.68 + Tailscale fallback) — located + Signal-sent Pravin's VVVV files. **Full post-show consolidation** (plan `~/.claude/plans/ok-so-the-shw-snuggly-tide.md`): (P1) **3090 → R&D idle** — stopped+disabled the live show stack, only SSH tunnel tasks remain; (P2) **archived IMPACT production stack** → `installation-archive/impact-2026/` (toe + 27 task XMLs + 19 scripts + README); (P3) **git consolidated** — 6 commits, ~494 files / +166K lines, hardened `.gitignore` (excluded ~70GB renders), pushed; (P4-6) **debrief-impact-2026.md** (operator-reviewed failure list) + **next-installation-improvements.md** (MOVE37XR) + CLAUDE.md/CURRENT_STATE refresh. (P7) **Visitor-dream consent archival executed LIVE** on salishseadreaming.art — added `ongoing` event window, flipped default scope, seeded 12 authored dreams, **archived the 63 non-consented-but-served IMPACT dreams** (retained, no longer served), published 3-dream consented snapshot. **Discovered the prod-deploy topology** the hard way: prod = manual nohup uvicorn on poly:9004 (ssd-v5), NOT the `ssd-gallery` systemd service (9000/v1, old schema) — restarting the wrong service + a stale in-process `event_membership` import cost debugging time. New memories: `reference_ssd_v5_prod_service_restart`, `project_3090_relocated_to_darrens_house`. Nothing deleted (349 DB rows; fresh checkpoint in private-archive). |
...
| `21dd2fa5` | 2026-06-23/24 | **BCAC grant support (LOC) + `.art` "About this project" page deploy** | Caught up on Pravin's grant asks (Signal Prav chat + Proton + `~/Downloads/art document.pdf`). Determined the urgent grant is **BCAC Media Artists (due Jun 24)**, not AGOG (already submitted). (1) The `art document.pdf` = **Jason Kitikul's signed LOC**, given as a template; produced **Darren's LOC** ($3,250, typed sig block) → `docs/grants/bcac-media-artists-2026/loc-darren-zal.{md,html,pdf}` + verifications sidecar; rendered to PDF via headless Chrome; operator sent it to Pravin himself. (2) Pravin also wanted an "about this project" section for the `.art` site and had **already authored the copy on his Canva page** — I first **mis-reported that page as empty (WebFetch can't render client-side Canva)**; corrected by rendering in a real browser (chrome-devtools). Discovered "the `.Art` website" = `salishseadreaming.art`, which lacked an about section. **Deployed live**: home intro blurb + "About this project" surface card + new **`/about/project`** canon page using Pravin's verbatim About-the-Artwork/Artist + his **canonical credits** (Austin **Aan'yas Harry**, Matt, Briony, Sandra Semchuk, Carol Anne, Shawn, **Eve=ecological-data**, Blair, Arshia, Leo, org partners). Mechanism: frontmattered md `ssd-v5:docs/about/about-the-artwork.md` wired via `_canon_dirs_map["project"]` + idempotent `index.html` patch (deploy script in scratchpad); **staged on v6 (9005) → promoted to v5 (9004)** per `reference_ssd_v5_prod_service_restart`; backups `.bak-about-20260623-2346`; verified file→origin→public-edge + browser screenshots; no regressions (mudra/ai/cloud/home 200). **Adversarial skeptic + as-built grep** caught and dropped a "tide/lunar/river drives the visuals" line (sourced to aspirational `podcast-source-document.md`, absent from the IMPACT debrief) and the weak "offered to the water" phrasing. Operator corrections honored: don't name KOI; herring/ecological data = Shawn+Eve, not Darren. Signal note to Pravin sent (ts `1782284758097`, via `--recipient` since `--chat "Prav"` was ambiguous with "Intro Prav to Jacob"). Updated memory `project_ssd_credit_attribution` (canonical credit list + `/about/<topic>` page-add recipe). **No git commit** (poly edited directly per deploy topology; repo `static/` is a known-stale mirror; BCAC packet + `docs/website/` UNTRACKED). |
| `5079a320` | 2026-06-12 | **AGOG Open Call 2026 grant — full application + per-place reframe + 3 adversarial workflows** | **SUBMITTED the AGOG "Climate Futures + Immersive Media" application** ($150K; applicant = Department of Bioregion US 501c3 EIN 85-4402939; Darren submitted via own Submittable). Built the full paste-ready packet in `docs/grants/agog-opencall-2026/` (form-fields, budget, deck, verifications sidecar, README, coord drafts). **Brandon (DOB) confirmed eligibility + sent EIN/990 + reviewed 👍.** Ran 3 multi-agent workflows (packet-QA, copy-truth-review, strategic-review) that **killed real fabrications I'd written**: the invented "Map of Silence" audio (real = Matt Robertson's herring catch/spawn-data musical score, sourced to the Signal Tactical chat), the coined term "Kwaxala," an unsourced "15% of visitors accept a commitment" metric; also fixed visuals-driven-by-spawn + "1888 vs 75yrs" + a LoRA-runs-live overclaim. **Reframed the whole proposal around per-place co-creation** (each of 8-10 Cascadia stops = a custom installation built with *local artists + local ecological data*; budget centers $55K local co-creation) — operator's key insight, grounded in the cosmo-local split + the 2005 *Islands in the Salish Sea Community Atlas* lineage + the host groups' bioregional mapping. **Voice → "witnessing is rewarded"** (ties Carol Anne's Indigenous witnessing framework; replaced an aspirational "stillness reveals hidden layers" mechanic that was NOT as-built — no room camera at the shows). Amount: $200→$175→$200→**$150K** (right-sized for a competitive $1M pool, per Brandon). Deck: 10 slides, em-dash-free, real show photos (via Shawn) + **Briony Penn's Salish Sea panorama** + the live 48-node dreamworld. **Seeded 36 authored, consent-clean dreams** (cloud 12→48) via the live `/prompt` pipeline on poly:9004; **fixed the home→dreamworld link** to the `ongoing` window (was pointing at the 3-node impact snapshot). Sent Pravin + Shawn Signal notes (Telegram session broken — Telethon SQLite "too many values to unpack"). Two required form fields (immersive-format, participant-experience) were absent from the June-11 form snapshot — drafted live during submit. Pravin's naming guidance applied (credit Austin+Matt; First Nations general; Coast Salish co-author unnamed). **Owed:** populate the new public repo `github.com/DarrenZal/SalishSeaDreaming` (linked in the grant, EMPTY) with reviewer-safe content only; send Brandon a PDF copy. Grant packet UNTRACKED (not committed). |
