...
- 🔴 **StreamDiffusion can silently stop — no process death, no alert (discovered 2026-04-17 23:50 PT).** TouchDesigner process stays up, Autolume keeps publishing NDI, Arena keeps rendering, but SD's internal server stops streaming. TD outputs the last rendered frame forever. `visitor.html` snapshot freezes on that frame. Every other "is the process alive?" probe returns green.
  - **Detection signal:** `op('/project1/StreamDiffusionTD').par.Streamactive.eval() == False` while TD process is alive. Also `par.Status` shows "Not Streaming" or "Server stopped..." and `par.Log` contains "Server stopped...".
  - **Heal recipe (proven 2026-04-17 23:50):** `op('/project1/StreamDiffusionTD').par.Startstream.pulse()` — restarts the SD server. TensorRT models take ~30–60 s to load, then `Status` flips to "X.XX fps, StreamDiffusionTD_*_out_*" and `Streamactive` becomes True.
  - **Remote path:** SSH port-forward `localhost:9981 → 3090:9981` (TD WebServer DAT), then TD MCP `execute_python_script`. One command: `ssh -fN -L 9981:127.0.0.1:9981 windows-desktop-remote`, then call TD MCP.
  - **Monitoring gap:** current `SSD-Health-Probe` only checks process-level liveness. Needs an SD-level probe (Phase 6 remainder of the auto-reboot-recovery plan).
